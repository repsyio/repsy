...
public interface ManifestService<ID> {
  ...
  void createManifestList(
      ID repoId, ID imageId, TagForm tagForm, List<ManifestListManifestInfo> manifestInfo);

  void createSinglePlatformManifest(ID repoId, BaseImageInfo<ID> imageInfo, TagForm tagForm);

  boolean findByDigestAndImageIdAndRepoId(String digest, ID imageId, ID repoId);
  ...
  List<? extends BaseManifestDetail<ID>> findManifests(ID repoId, ID imageId, ID tagId);
}
