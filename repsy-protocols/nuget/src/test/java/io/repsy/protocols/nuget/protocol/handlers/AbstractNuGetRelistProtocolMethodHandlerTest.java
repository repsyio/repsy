...
@DisplayName("AbstractNuGetRelistProtocolMethodHandler")
class AbstractNuGetRelistProtocolMethodHandlerTest {
  ...
  private AbstractNuGetRelistProtocolMethodHandler handler;
  ...
  static class TestHandler extends AbstractNuGetRelistProtocolMethodHandler {
  ...
  void pathParserMatchesEndpoint(final String path, final boolean matches) {
    final var request = request("POST", path);
  ...
  @Test
  @DisplayName("handle() returns 200 OK when the facade succeeds")
  void handlesSuccess() throws IOException {
    ...
    verify(facade).relistVersion(ctx);
  ...
  void handlesNotFound() throws IOException {
    final var ctx = context(PATH);
    doThrow(new ItemNotFoundException("packageNotFound")).when(facade).relistVersion(ctx);
  ...
  void handlesUnexpectedFailure() throws IOException {
    final var ctx = context(PATH);
    doThrow(new IllegalStateException("db down")).when(facade).relistVersion(ctx);

    final var response =
        handler.handle(ctx, new MockHttpServletRequest(), new MockHttpServletResponse());

    assertThat(response.getStatusCode()).isEqualTo(HttpStatus.INTERNAL_SERVER_ERROR);
    assertThat(response.getBody()).isEqualTo(NuGetErrorResponse.of("Relist failed"));
  }
}
