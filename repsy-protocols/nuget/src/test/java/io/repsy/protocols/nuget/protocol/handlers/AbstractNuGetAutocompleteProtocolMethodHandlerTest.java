 ...
 * had no upper bound. Autocomplete's over-fetch also added {@code skip + take} without overflow
 * protection.
 */
@ExtendWith(MockitoExtension.class)
@DisplayName("AbstractNuGetAutocompleteProtocolMethodHandler")
class AbstractNuGetAutocompleteProtocolMethodHandlerTest {

  private static final String AUTOCOMPLETE_PATH = "/nuget/v3/autocomplete";

  @Mock private PathParser basePathParser;
  @Mock private NuGetProtocolFacade facade;
  @Mock private NuGetProtocolProvider provider;

  private AbstractNuGetAutocompleteProtocolMethodHandler handler;

  private final ListAppender<ILoggingEvent> logs = new ListAppender<>();
  private final Logger logger =
      (Logger) LoggerFactory.getLogger(AbstractNuGetAutocompleteProtocolMethodHandler.class);

  static class TestHandler extends AbstractNuGetAutocompleteProtocolMethodHandler {
  ...
  private static MockHttpServletRequest request(final String query) {
    final var request = new MockHttpServletRequest("GET", AUTOCOMPLETE_PATH);
    request.setServletPath(AUTOCOMPLETE_PATH);
    if (query != null) {
