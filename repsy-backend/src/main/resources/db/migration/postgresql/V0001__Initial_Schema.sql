-- Repsy OS - Initial Database Schema - PostgreSQL
...
CREATE UNIQUE INDEX "ux_npm_package__repo_id_scope_name" ON "npm_package" ("repo_id", COALESCE("scope", ''), "name");
...
CREATE UNIQUE INDEX "ux_docker_image__repo_id_name" ON "docker_image" ("repo_id", COALESCE("name", ''));
...
-- Partial index: allows multiple orphan manifests (tag_platform_id IS NULL)
CREATE UNIQUE INDEX "ux_docker_manifest__tag_platform_digest"
    ON "docker_manifest" ("tag_platform_id", "digest")
    WHERE "tag_platform_id" IS NOT NULL;
