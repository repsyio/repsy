...
(
    ...
    constraint "ux_helm_chart__repo_id_name"
        unique ("repo_id", "name")
);
...
create table "public"."helm_chart_version"
(
    "id"              uuid        not null
        constraint "pk_helm_chart_version"
            primary key,
    "version_lock"    integer     not null,
    ...
    constraint "ux_helm_chart_version__chart_id_version"
        unique ("chart_id", "version")
);

alter table "public"."helm_chart_version"
    add constraint "fk_helm_chart_version__chart_id"
        foreign key ("chart_id") references "public"."helm_chart" ("id") on delete cascade;

create index "ix_helm_chart_version__chart_id"
    on "public"."helm_chart_version" ("chart_id");
...
(
    ...
    "created_at"       timestamp    not null,
    "last_updated_at"  timestamp,
    constraint "ux_helm_oci_manifest__repo_id_name_reference"
        unique ("repo_id", "name", "reference")
);
...
alter table "public"."helm_oci_manifest"
    add constraint "fk_helm_oci_manifest__chart_version_id"
        foreign key ("chart_version_id") references "public"."helm_chart_version" ("id") on delete cascade;

create index "ix_helm_oci_manifest__repo_id"
    on "public"."helm_oci_manifest" ("repo_id");

create index "ix_helm_oci_manifest__chart_version_id"
    on "public"."helm_oci_manifest" ("chart_version_id");
