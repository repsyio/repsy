...
@NullMarked
public class RubyPathParser implements PathParser {
  ...
  public Optional<ProtocolContext> parse(final HttpServletRequest request) {
    ...
    final var repoInfoOpt = this.repoTxService.getRepoByNameAndType(repoName, RepoType.RUBY);

    return repoInfoOpt.flatMap(repoInfo -> this.createContext(repoInfo, repoName, matcher));
  }

  private Optional<ProtocolContext> createContext(
