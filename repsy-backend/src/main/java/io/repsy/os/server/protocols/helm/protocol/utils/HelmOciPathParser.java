...
@Component("osHelmOciPathParser")
@RequiredArgsConstructor
@NullMarked
public class HelmOciPathParser implements PathParser {
  ...
  private static final Pattern PATH_PATTERN =
      Pattern.compile("^/v2/" + REPO_NAME_REGEX + RELATIVE_PATH_REGEX);
  ...
  public Optional<ProtocolContext> parse(final HttpServletRequest request) {
    ...
    final var matcher = PATH_PATTERN.matcher(path);
  ...
  private Optional<ProtocolContext> createProtocolContext(
      final io.repsy.os.shared.repo.dtos.RepoInfo repoInfo,
