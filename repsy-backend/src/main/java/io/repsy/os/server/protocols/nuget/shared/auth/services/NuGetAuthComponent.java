...
import static io.repsy.protocols.shared.repo.dtos.RepoType.NUGET;
...
@Component
@NullMarked
public class NuGetAuthComponent extends ProtocolAuthService {

  public NuGetAuthComponent(
  ...
  private void processAuthRequest(
    ...
    if (isBasicToken(authHeader)) {
      userInfo = this.resolveBasicAuthUser(authHeader);
    } else {
      userInfo = this.resolveBearerAuthUser(repoInfo, authHeader);
    }
  ...
  private Optional<String> authenticateWithDeployToken(final Credentials credentials) {
    ...
    final var deployTokenOpt =
        this.deployTokenService.findByTokenAndRepoType(credentials.getPassword(), NUGET);
