...
export class NpmComponent implements OnInit, OnDestroy {
  ...
  public ngOnInit(): void {
    ...
    this.repoSubscription = this.repoLookupService.currentRepo$
      .pipe(filter((repo): repo is RepoContext => repo !== null && repo.repoType === 'npm'))
      .subscribe((repoContext) => {
        this.loadRegistry(repoContext.repoName);
      });

    const currentRepo = this.repoLookupService.currentRepo;
    if (currentRepo?.repoType === 'npm') {
      this.loadRegistry(currentRepo.repoName);
    }
  }
