...
@Component({
  selector: 'app-package-security-modal',
  standalone: true,
...
export class PackageSecurityModalComponent implements OnChanges {
  ...
  public ngOnChanges(changes: SimpleChanges): void {
    if (changes['open'] && this.open && this.repoName && this.artifactName) {
      this.fetchDetail();
    }
  }
  ...
  public viewAllVersions(event: Event): void {
    ...
    if (this.packageQueryParams) {
      this.router.navigate([this.packageRoute], { queryParams: this.packageQueryParams });
    } else {
      this.router.navigateByUrl(this.packageRoute);
    }
  }
  ...
  private fetchDetail(): void {
    ...
    this.securityService
      .getArtifactSecurityDetail(this.repoName, this.artifactName)
