...
cmd_test() {
  ...
  if [ -n "$protocols" ]; then
    ...
    # cargo, nuget, docker, helm, and more from later steps) is opt-in via --protocol so a plain
    # "run.sh test" stays fast; pass e.g. --protocol maven or
    # --protocol skeleton,maven,npm,cargo,nuget,docker,helm to run more. "helm" runs BOTH Helm
