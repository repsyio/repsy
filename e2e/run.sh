...
cmd_test() {
  ...
  if [ -n "$protocols" ]; then
    ...
    # cargo, nuget, and more from later steps) is opt-in via --protocol so a plain "run.sh test"
    # stays fast; pass e.g. --protocol maven or --protocol skeleton,maven,npm,cargo,nuget to run more.
    services=(skeleton)
  fi
