...
/**
 * The helm client's environment is an allow-list (the OCI and the classic ChartMuseum mode share it).
...
import { helmEnv } from '../../src/clients/helm.js';

test(
  'helm > no runner variable reaches helm, its directories and plugin path do',
  { tag: ['@sealed-env'] },
  async () => {
    const { home, work } = await isolatedWorkDir('helm-sealed-env');
    const seen = await probeEnv(helmEnv(home), work, 'helm-sealed-env');
    expectSealed(seen, home, ['HELM_CACHE_HOME', 'HELM_PLUGINS']);
  },
);
