...
/**
 * The dotnet/nuget client's environment is an allow-list.
...
import { nugetEnv } from '../../src/clients/nuget.js';

test(
  'nuget > no runner variable reaches dotnet, its cache directories do',
  { tag: ['@sealed-env'] },
  async () => {
    const { home, work } = await isolatedWorkDir('nuget-sealed-env');
    const seen = await probeEnv(nugetEnv(home), work, 'nuget-sealed-env');
    expectSealed(seen, home, ['DOTNET_CLI_HOME', 'NUGET_PACKAGES']);
  },
);
