 ...
 *    not exist;
 *  - `dist-tag add` succeeds for every client, yarn classic included (RPS-1362: the registry answers
 *    JSON with `ok`, which yarn 1 requires).
 */
...
for (const client of clientsWith('distTagCmd')) {
  test(
    ...
    async ({ seeder }) => {
      ...
      // Every client, yarn 1 included, sees the add succeed: the answer is JSON with an `ok` field
      // (RPS-1362; yarn classic took the empty answer for "Couldn't add tag" and exited 1).
      expect(moved.exitCode, `dist-tag add: ${moved.command}\n${moved.stderr}`).toBe(0);
