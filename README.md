...
- **Multi-Protocol Support**: Docker, Golang, Cargo (Rust), Maven, npm, PyPI, NuGet registries
...
docker run -d \
  ...
  -p 8080:8080 \
  -p 9090:9090 \
  repo.repsy.io/repsy/os/repsy:26.05.0
```
...
services:
  ...
  repsy:
    ...
    image: repo.repsy.io/repsy/os/repsy:26.05.0
    depends_on:
      - postgres
    ports:
      - "8080:8080"
...
- **Admin Initial Password**: Only applied when no admin user exists in the database. After the first run, change your password through the application interface.
...
For detailed information on creating repositories, managing deploy tokens, and using different protocols (Docker, Golang, Cargo(Rust), Maven, npm, PyPI, NuGet), see the [documentation](https://docs.repsy.io).
